...
bool JointDynamixel::setSDKHandler(uint8_t actuator_id)
{
  ...
  //--------------수정부분----------------------
  //기존 e manual은 manipulator plus의 프로토콜 주소를 기반으로 제작되었기 때문에 manipulator pro를 사용하기 위해 주소를 변경해야한다.
  //manipulator pro를 동작하기 위해서는 포지션,벨로시티만을 사용해야한다.
  ...
  result = dynamixel_workbench_->addSyncReadHandler(ADDR_PRESENT_POSITION_2, 
                                                    (LENGTH_PRESENT_VELOCITY_2 + LENGTH_PRESENT_POSITION_2), 
                                                    &log);
  if (result == false)
  {
    log::error(log);
...
std::vector<robotis_manipulator::ActuatorValue> JointDynamixel::receiveAllDynamixelValue(std::vector<uint8_t> actuator_id)
{
  ...
  //--------------수정부분----------------------
  //getsyncreadData는 present current 부분을 제외하고 주소의 위치를 맞춰서 작성한다.
