...
bool SolverUsingCRAndGeometry::inverseSolverUsingGeometry(Manipulator *manipulator, Name tool_name, Pose target_pose, std::vector<JointValue> *goal_joint_value)
{
  ...
  Eigen::MatrixXd orientation3 = math::convertRPYToRotationMatrix(0,0,target_angle[0].position);    //controller에서 받은 target_pose의 x,y,z의 데이터를 가져온다
  ...
  double l1 = sqrt(0.264*0.264 + 0.030*0.030);          //openmanipulator h link length 264cm
  double l2 = sqrt(0.030*0.030 + 0.258*0.258);          //openmanipulator h link length 258cm
  double phi = acos((l1*l1 + l2*l2 - position3_4.norm()*position3_4.norm()) / (2*l1*l2));       //x^2 + y^2 - l1^2 -l2^2 / 2*l1*l2 역기구학의 공식 적용됨
