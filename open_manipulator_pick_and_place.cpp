...
//ar marker의 좌료를 기준으로 기구학으로 경로 생성 및 작업공간 표시
...
//ar marker의 위치값인 x,y,z 의 값을 가져온다
...
{
  double radian;
  if(dgree >= 0.0 && dgree<=360.0)
  {
    radian = (dgree * PI) / 180.0;
  }
  else if (dgree < 0.0 && dgree >= -360.0)
  {
    radian = (dgree * PI) / 180.0;
  }
...
void OpenManipulatorPickandPlace::publishCallback(const ros::TimerEvent&)
{
  ...
  if (mode_state_ == HOME_POSE)
  {
    ...
    // joint_angle.push_back(-PI/4);    //-45도
    joint_angle.push_back( d2r(-45) );
    // joint_angle.push_back(PI/8);    //22.5도
    joint_angle.push_back( d2r(22.5) );    //22.5도
    joint_angle.push_back(0.0);
    // joint_angle.push_back(PI/8);    //22.5도
    joint_angle.push_back( d2r(22.5) );    //22.5도
    joint_angle.push_back(0.0);
...
void OpenManipulatorPickandPlace::demoSequence()
{
  ...
  {
  case 0: // home pose
    joint_angle.push_back(d2r(0));
    // joint_angle.push_back(-PI/4);   //45도
    joint_angle.push_back(d2r(-45));   //45도
    joint_angle.push_back(PI/8);    //15도
    joint_angle.push_back(0.0);
  ...
  case 1: // initial pose
    ...
    joint_angle.push_back( -0.40);
    joint_angle.push_back( 0.25);
    joint_angle.push_back( 0.00);
    joint_angle.push_back( 1.30);
    joint_angle.push_back( 0.00);
  ...
  //       kinematics_position.push_back(0.5);
  //       kinematics_position.push_back(0.01);
  //       kinematics_position.push_back(0.01);
  //       kinematics_orientation.push_back(0.74);
  //       kinematics_orientation.push_back(0.00);
  //       kinematics_orientation.push_back(0.66);
  ...
  case 5: // initial pose
    ...
    demo_count_ ++;
    break;
  case 6: // place pose 목표 지점으로 회전한다
    joint_angle.push_back( 1.50);
    joint_angle.push_back(-0.21);
    joint_angle.push_back(-0.15);
    joint_angle.push_back( 0.00);
    joint_angle.push_back( 1.40);
    joint_angle.push_back( 0.00);
    setJointSpacePath(joint_name_, joint_angle, 1.0);
    demo_count_ ++;
    break;

  //여기부터 오류가 발생한다. 오류 해결방법 quaternion에서 euler로 변환하는 방법을 조사한다
  // w=0.74 x=0.00 y=0.66 z=0.00 인데 이는 그리퍼의 각도를 정한다 수직으로 내려보는 각도
  case 7: // place the box 목표 지점으로 내려간다
    kinematics_position.push_back(present_kinematic_position_.at(0)+0.1);
    kinematics_position.push_back(present_kinematic_position_.at(1));
    if (pick_ar_id_ == 0)  kinematics_position.push_back(0.08);  
  ...
  //   kinematics_position.push_back(present_kinematic_position_.at(0)+0.1);
  //   kinematics_position.push_back(present_kinematic_position_.at(1));
  //   kinematics_position.push_back(0.08);
  //   kinematics_orientation.push_back(0.74);
  //   kinematics_orientation.push_back(0.00);
  //   kinematics_orientation.push_back(0.66);
  //   kinematics_orientation.push_back(0.00);
  ...
  case 8: // wait & place  gripper를 연다
    setJointSpacePath(joint_name_, present_joint_angle_, 1.0);
    gripper_value.push_back(0.04);
    setToolControl(gripper_value);
    demo_count_ ++;
    break;
  case 9: // move up after place the box 목표 지점에서 올라간다
    kinematics_position.push_back(present_kinematic_position_.at(0));
    kinematics_position.push_back(present_kinematic_position_.at(1));
    kinematics_position.push_back(0.2);
    // std::cout << present_kinematic_position_.at(2)+0.150<<"\n";
    kinematics_orientation.push_back(0.74);
    kinematics_orientation.push_back(0.00);
    kinematics_orientation.push_back(0.66);
    kinematics_orientation.push_back(0.00);
